...
internal class SushiScanFRFullVerificationTest {

	@Test
	fun verifySushiScanFRParserEndToEnd() = runTest(timeout = 6.minutes) {
		val parser = MangaLoaderContextMock.newParserInstance(MangaParserSource.SUSHISCANFR)
		...
		val sampleQuery = updated.first().title
			.split(Regex("\\s+"))
		...
		val search = parser.getList(
			...
			filter = MangaListFilter(query = sampleQuery),
		)
		check(search.isNotEmpty()) { "Search returned empty list for query '$sampleQuery'" }
		...
		println(
			"SUSHISCANFR_FULL_SMOKE|updated=${updated.size}|alpha=${alphabetical.size}|search=${search.size}|" +
				"manga=${detailed.publicUrl}|chapters=${chapters.size}|samples=$sampledSummary",
		)
	}

	private suspend fun checkImageResponse(url: String?) {
		check(!url.isNullOrBlank()) { "Image URL is blank" }
		MangaLoaderContextMock.doRequest(url, MangaParserSource.SUSHISCANFR).use { response ->
