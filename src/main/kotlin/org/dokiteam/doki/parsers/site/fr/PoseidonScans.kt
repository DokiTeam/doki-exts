...
internal class PoseidonScans(context: MangaLoaderContext) :
	...
	private suspend fun buildChapterSeedCandidates(slug: String): List<ChapterSeed> {
		...
		addCandidate(
			allMangaCache.get().firstOrNull { it.manga.url == "/serie/$slug" }?.chaptersCount
	...
	override suspend fun getDetails(manga: Manga): Manga {
		...
		val slug = manga.url.substringAfter("/serie/").substringBefore("/").ifBlank {
			manga.publicUrl.substringAfter("/serie/").substringBefore("/")
		}
		val cachedManga = if (slug.isBlank()) {
			null
		} else {
			allMangaCache.get().firstOrNull { it.manga.url == "/serie/$slug" }?.manga
		}
		...
		val fallbackChapters = if (slug.isBlank()) emptyList() else recentChaptersCache.get()[slug].orEmpty()
		return base.copy(
			description = base.description ?: "",
